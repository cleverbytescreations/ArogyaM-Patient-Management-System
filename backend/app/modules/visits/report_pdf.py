"""HTML -> PDF rendering for the case sheet report (BE-T6.4).

Renders the Jinja2 template in templates/case_sheet.html through WeasyPrint.
Header logos are embedded as base64 data URIs so the PDF needs no filesystem
or network access at render time and looks identical everywhere.
"""
...
def render_case_sheet_pdf(context: dict[str, Any]) -> bytes:
    """Render the case sheet context dict to PDF bytes."""
    template = _env.get_template("case_sheet.html")
